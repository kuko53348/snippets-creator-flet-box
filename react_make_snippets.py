import json
from ReactNative.ReactNative_snippets_creator.ReactNative_create_attributes_widgets import (
    attrs_all_placeholders_complete,
)
from ReactNative.ReactNative_snippets_creator.ReactNative_create_widgets import (
    only_controls,
)
from ReactNative.ReactNative_snippets_creator.ReactNative_create_modules_widgets import (
    only_modules,
)
...
class make_snippet:
    ...
    """
    instance = make_snippet(path='./ReactNative', file_name='ReactNative.json')
    ```
    ...
    def __init__(
        self, path: str = "./ReactNative", file_name: str = "ReactNative.json"
    ):
        self.path = path
    ...
    def write_json(self, data_to_write: dict = {}, tag_name: str = "tag-name"):
        ...
        print(f"[*] - SNIPPET WAS CREATE {tag_name} ReactNative.json")
...
if __name__ == "__main__":
    ...
    # texto = """
    # """
    # instance = make_snippet(path="./ReactNative", file_name="ReactNative_widgets_ReactNative.json")
    ...
    instance = make_snippet(
        path="./ReactNative/", file_name="ReactNative_attribute_widgets.json"
    )
    check_value={}
    # print(attrs_all_placeholders_complete)
    # MAKE SNIPPETS FROM DICT
    # ReactNative.ReactNative_SNIPPETS_CREATOR.ReactNative_CREATE_ATTRIBUTES_WIDGETS IMPORT
    for index, value in attrs_all_placeholders_complete.items():
        name = f"ReactNative widgets {index}"
        prefix = f"{index}"
    ...
    instance.write_json(data_to_write=old_data, tag_name="ReactNative-attributes")

    # ==================== CREATE WIDGETS WIDGETS ======================

    instance = make_snippet(path="./ReactNative/", file_name="ReactNative_widgets.json")
    # print(attrs_all_placeholders_complete)
    # MAKE SNIPPETS FROM DICT
    # ReactNative.ReactNative_SNIPPETS_CREATOR.ReactNative_CREATE_ATTRIBUTES_WIDGETS IMPORT
    ...
    del old_data[""]
    old_data.update(check_value)
    instance.write_json(data_to_write=old_data, tag_name="ReactNative-widgets")
    # ==================== CREATE WIDGETS MODULES WIDGETS ======================

    instance = make_snippet(
        path="./ReactNative/", file_name="ReactNative_modules_widgets.json"
    )
    # print(attrs_all_placeholders_complete)
    # MAKE SNIPPETS FROM DICT
    # ReactNative.ReactNative_SNIPPETS_CREATOR.ReactNative_CREATE_ATTRIBUTES_WIDGETS IMPORT
    for index, value in only_modules.items():
        name = f"ReactNative modules widgets {index}"
        prefix = f"{index}"
        description = f"widget {index.replace('_', ' ')}"
        check_value = instance.read_original_to_write_json(
            name=name,
    ...
    del old_data[""]
    old_data.update(check_value)
    instance.write_json(data_to_write=old_data, tag_name="ReactNative-modules-widgets")
    # # ==================== DUMP ALL IN ReactNative.json=====================

    # ATTRIBUTES
    instance_attributes = make_snippet(
        path="./ReactNative/",
        file_name="ReactNative_attribute_widgets.json",
    )
    # WIDGETS
    instance_widgets = make_snippet(
        path="./ReactNative/",
        file_name="ReactNative_widgets.json",
    )
    # MODULES
    instance_modules_widgets = make_snippet(
        path="./ReactNative/",
        file_name="ReactNative_modules_widgets.json",
    )

    # WIDGETS in ReactNative.js FINAL
    instance_final_widgets = make_snippet(
        path="./ReactNative/",
        file_name="ReactNative.json",
    )
    ...
    instance_final_widgets.write_json(
        data_to_write=all_data, tag_name="ReactNative-all-together"
    )
    # print(all_data)
