...
export const MyComponent = ({ title, onPress }) => {
    ...
    'createComponent.listview': '''import { ListView, ListTile, Avatar, createList, random } from 'flet-box';
...
export const MyList = ({ data = generateData(), onSelect }) => {
    ...
    'createComponent.gridview': '''import { GridView, Card, Column, Image, Text, createList, random } from 'flet-box';
...
export const MyGallery = ({ items = generateImages(), onPress }) => {
    ...
    # =========================================================================
    # WIDGET TEMPLATES (createWidget)
    # =========================================================================
    'createWidget.basic': '''import { Container, Text, colors } from 'flet-box';

const MyWidget = ({ text, ...props }) => {
    return Container({
        padding: 16,
        bgColor: colors.surface,
        borderRadius: 12,
        child: Text({ text: text || "My Widget" }),
        ...props
    });
};
...
export default MyWidget;''',
    ...
    'createWidget.stateful': '''import { Container, Column, Text, Button, colors, useState } from 'flet-box';
...
const CounterWidget = () => {
    ...
    return Container({
        ...
        child: Column({
            gap: 12,
            children: [
                Text({ text: `Count: ${count}`, size: 20, weight: "bold" }),
                Button({
                    text: "Increment",
...
export default CounterWidget;''',
    ...
    'createWidget.withProps': '''import { Container, Column, Text, Button, colors } from 'flet-box';

const ProfileWidget = ({ user, onLogout }) => {
    return Container({
        padding: 16,
        bgColor: colors.surface,
        borderRadius: 12,
        child: Column({
            gap: 8,
            children: [
                Text({ text: user.name, size: 18, weight: "bold" }),
                Text({ text: user.email, color: colors.textSecondary }),
                Button({
                    text: "Logout",
                    variant: "outlined",
                    onPress: () => onLogout(user.id)
                })
            ]
        })
    });
